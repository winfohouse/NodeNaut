<script lang="ts">
  import { Globe, Zap, ExternalLink } from '@lucide/svelte';
  export let node: any;
  export let preview: string | null = null;
</script>

<div class="node-view navigate-view">
  <header class="node-header">
    <div class="icon-box">
      <Globe size={14} />
    </div>
    <div class="name-col">
      <span class="node-name">{node.metadata?.label || 'Navigate URL'}</span>
      <span class="node-meta-tiny">{node.id.slice(0, 8)} • BROWSER</span>
    </div>
  </header>
  
  <div class="node-body">
    <div class="data-row">
      <ExternalLink size={10} class="text-muted" />
      <code class="url-peek">{node.state.url || 'No URL'}</code>
    </div>
    
    <div class="data-row status-row">
      <Zap size={10} class="icon-accent" />
      <span class="type-text">Target</span>
      {#if preview}
        <div class="live-pill">
          <span>{preview}</span>
...
<style>
  ...
  .url-peek { font-size: 0.55rem; color: var(--text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
