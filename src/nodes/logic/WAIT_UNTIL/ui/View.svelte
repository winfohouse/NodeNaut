<script lang="ts">
  import { Timer, Zap, Code } from '@lucide/svelte';
  export let node: any;
  export let preview: string | null = null;
</script>

<div class="node-view wait-until-view">
  <header class="node-header">
    <div class="icon-box">
      <Timer size={14} />
    </div>
    <div class="name-col">
      <span class="node-name">{node.metadata?.label || 'Wait Until'}</span>
  ...
  <div class="node-body">
    <div class="data-row">
      ...
      <code class="expr-peek">{node.state.value || 'No condition'}</code>
    </div>
    
    <div class="data-row status-row">
      <Zap size={10} class="text-warning" />
      <span class="type-text">Dynamic Polling</span>
      {#if preview}
        <div class="live-pill">
          <span>{preview}</span>
...
<style>
  ...
  .expr-peek { font-size: 0.55rem; color: var(--text-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
  ...
  .live-pill { display: flex; align-items: center; gap: 0.3rem; background: var(--accent-glow); padding: 0.1rem 0.3rem; border-radius: 4px; border: 1px solid var(--accent); max-width: 100px; }
